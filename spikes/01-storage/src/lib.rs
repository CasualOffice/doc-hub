//! Spike #1 — Storage facade over `opendal::Operator`.
//!
//! Scope: prove the facade shape from `docs/ARCHITECTURE.md` compiles + the
//! conformance suite (fs + memory) passes. MinIO via testcontainers is optional
//! and gated on Docker availability. S3 is out of scope here.
//!
//! Decision deliverable: confirm or revise the trait shape, the `SignedUrl`
//! enum, and the capability-gate branch. Anything that hurts here gets fixed
//! before Phase 1 hardens it into `crates/drive-storage`.
...
#[derive(Debug, Clone)]
pub enum SignedUrl {
    /// Backend issued a native presigned URL (S3 / MinIO). Browser hits the
    /// backend directly.
    Native {
        url: url::Url,
        expires_at: time::OffsetDateTime,
    },
    /// We minted an HMAC token. Served via the `/raw/{token}` route on the
    /// user-content origin.
    Token {
        token: String,
        expires_at: time::OffsetDateTime,
...
impl Storage {
    /// Build a Storage over a pre-configured OpenDAL Operator + an HMAC secret.
    ...
    pub fn fs(root: impl Into<String>, sign_key: [u8; 32]) -> Result<Self, StorageError> {
        let op = opendal::Operator::new(opendal::services::Fs::default().root(&root.into()))?
            .finish();
    ...
    /// S3-protocol Storage (works for AWS S3 + MinIO via `endpoint`).
    pub fn s3(
        bucket: &str,
        region: &str,
        endpoint: Option<&str>,
    ...
    pub async fn put(
        ...
        validate_key(key)?;
        // OpenDAL 0.54's writer-with-content-type API moves around between
        // patches; for the spike we go through `write` which sets a default
        // content-type. Phase 1 hardens this to a typed Writer.
    ...
    /// Copy `src` to `dst`. Falls back to read-then-write on backends that
    /// don't support native copy (e.g. the memory service in OpenDAL 0.54).
    ...
    /// Rename `src` to `dst`. Falls back to copy-then-delete on backends that
    /// don't support native rename. The copy itself may further fall back.
    ...
    pub async fn list(
        ...
        // Spike: read the whole listing eagerly. Phase 1 honours `page_token`
        // via OpenDAL's `lister_with(...).limit(N)`.
        let entries = self
            .op
            .list(prefix)
            .await?
    ...
    /// Issue a signed download URL. Branches on the backend's presign capability.
    pub async fn signed_get(
        ...
        if self.capabilities().presign_read {
            ...
            let url = url::Url::parse(&req.uri().to_string())
                .map_err(|_| StorageError::Backend(opendal::Error::new(
                    opendal::ErrorKind::Unexpected, "presign returned non-URL")))?;
    ...
    /// Issue a signed upload URL (mirror of signed_get).
