...
// the same source. PIPELINE: MK-PERF.
...
async function run() {
  ...
  for (const png of pngs) {
    ...
    const [webpFresh, avifFresh] = await Promise.all([
    ...
    totalWebpBytes += (await fs.stat(webp)).size;
    totalAvifBytes += (await fs.stat(avif)).size;
  }
