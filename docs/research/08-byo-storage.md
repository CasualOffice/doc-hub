...
| **SSRF via test-connection** | Endpoint must be `https://` (with `http://` allowed only when host is `localhost`/`127.0.0.1`/private RFC1918 *and* `DOCHUB_ALLOW_INSECURE_BYO=true`). Two passes: (1) `ssrf_guard` — static checks on scheme, a metadata block list, and IP-literal ranges; (2) `ssrf_guard_resolving` — in the default posture, **resolves the hostname and refuses if any address is a private/loopback/link-local/metadata IP**, closing the bypass where a public-looking name (`metadata.internal`) resolves to `169.254.169.254`. `allow_insecure` skips pass 2 (operator opted into private targets). Residual — DNS rebinding (a name that passes pass 2, then re-resolves to an internal IP at connect time) — is closed at the **network layer**, the authoritative SSRF control: production deploys MUST deny the app's egress to loopback/RFC1918/link-local/metadata ranges (see RUNBOOK → *Network egress hardening*). A socket-level resolver guard was evaluated and rejected: swapping OpenDAL's HTTP client is unverifiable in CI (the guard blocks localhost, so the S3 happy path can't be smoke-tested; no live S3 in CI) and network policy covers all egress paths, not just BYO. |
